...
if __name__ == '__main__':
    ...
    for yq_repo in yq_repo_list:
        print("获取【{}】仓库下的文档，文档ID【{}】".format(yq_repo.repo_name, yq_repo.repo_id))
        # fetch_toc_list(yq_repo.repo_id, yq_repo.repo_name)
    if len(yq_repo_list) == 0:
        exit('没找到文档')
    repo_id = input('请选择要备份的仓库：')
    repo_name = input('请输入备份后的仓库名称：')
    fetch_toc_list(repo_id, repo_name)
